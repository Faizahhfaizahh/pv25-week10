...
class MainWindow(QMainWindow):
    ...
    def setupDatabase(self):
        self.conn = sqlite3.connect("data_buku.db")
    ...
    def simpanData(self):
        ...
        if not all([judul, penulis, tahun, kategori, tempat]):
        ...
        if judul and penulis and tahun and kategori and tempat:
            self.cursor.execute("INSERT INTO buku (judul, penulis, tahun, kategori, tempat) VALUES (?, ?, ?, ?, ?)",
                                (judul, penulis, tahun, kategori, tempat))
            self.conn.commit()
            self.tampilkanData()
    ...
    def edit_data_buku(self, row, column):
        ...
        kolom_nama = ["ID", "Judul", "Penulis", "Tahun", "Kategori", "Tempat Terbit"]
        kolom_field = ["judul", "penulis", "tahun", "kategori", "tempat"]
        ...
        if filename:
            try:
                with open(filename, mode='w', newline='', encoding='utf-8') as file_csv:
                    ...
                    headers = ["ID", "Judul Buku", "Penulis", "Tahun Terbit", "Kategori", "Tempat Terbit"]
